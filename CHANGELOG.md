...
- `CPI001-P` (`arbitrary-cpi`, `warn`): flags an `invoke`/`invoke_signed` whose `program_id` is taken from a caller-supplied account whose key is never compared to an expected id, letting an attacker redirect the CPI to malicious code. The fact-table engine now records the program-id source of each raw invoke and whether the program account's key was actually compared (via `==`/`!=` or an assert/require macro), distinct from merely being read; typed-builder CPIs (`CreateAccount { … }.invoke_signed(…)`), constant/param program ids, and delegated program accounts are all left alone. Ships at `warn` because the syn-only pass cannot see cross-function key checks; promote with `--deny CPI001-P`.
- `ACC002-P` (`missing-signer`, `warn`): flags an account whose key is checked against a stored authority field (`authority.address() == state.authority`, matched via `==`/`!=` or `.eq()`/`.ne()`) but is never `is_signer()`-checked, so anyone can act as that authority by passing its public address. The engine now marks a binding as an authority position when its key is compared against an authority-named field/var (`authority`, `admin`, `auth`, or a `*_authority`/`*_auth` suffix). Ships at `warn` because the syn-only pass cannot see a signer enforced in a helper or downstream token CPI; a delegated account is left alone. Promote with `--deny ACC002-P`.
- `ACC003-P` (`account-confusion`, `warn`/`heuristic`): flags an account named like a global singleton (`config`, `settings`, `*_config`, `*_settings`) read as trusted state without its key ever being compared to an expected address, letting an attacker substitute a look-alike account with attacker-chosen contents. Ships at `heuristic`, so it is hidden at the default `likely` threshold (surface it with `confidence_threshold = "heuristic"` or `--deny ACC003-P`), because a syn-only pass cannot tell a true singleton from a per-instance account (per-user, per-pool, per-position) whose key legitimately varies and is validated by deriving its PDA. A delegated account is left alone. Promoting to `likely` waits on the dylint backend resolving the PDA derivation.
