    ...
    match &cli.command {
        ...
        Commands::Client { command } => match command {
            ClientCommands::Generate {
                ...
                generate_client(idl_dir, out_dir, *generator, *auto_install)?;
            }
        },
        Commands::Help => {
            display_help_banner()?;
...
fn generate_idl(out_dir: &str) -> Result<()> {
    ...
    let codama_idl = idl_codama::to_codama_compatible(&idl);
    let codama_idl_json = codama_idl
        .try_into_json()
...
fn generate_client(
    ...
    match generator {
        Generator::Shank => {
            ...
            client_gen::generate_rust_client(&idl_path, Path::new(out_dir))
        ...
        Generator::Codama => {
            ...
            codama_gen::generate_via_codama(&idl_path, Path::new(out_dir), auto_install)
