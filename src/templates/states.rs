...
impl MyState {
    ...
    pub fn validate_pda(bump: u8, pda: &Address, owner: &Address) -> Result<(), ProgramError> {
        let seed_with_bump = &[Self::SEED.as_bytes(), owner.as_array().as_slice(), &[bump]];
    ...
    pub fn initialize(my_stata_acc: &mut AccountView, ix_data: &Initialize) -> ProgramResult {
        let my_state =
